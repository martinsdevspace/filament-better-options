...
<x-dynamic-component :component="$fieldWrapperView" :field="$field" class="fi-fo-radio-cards-wrapper">
  <div {{
    ...
    @foreach ($getOptions() as $value => $label)
      ...
      <label @class([
        ...
        'is-centered' => $isItemsCenter(),
                  ...
                  }" :class="{ 'is-selected': isSelected }" role="radio" :aria-checked="isSelected"
        :aria-selected="isSelected" for="{{ $itemId }}" :aria-disabled="{{ $isDisabled ? 'true' : 'false' }}"
        tabindex="0">
        @if ($isIndicatorBefore() && $isIndicatorVisible())
          <x-better-options::option-indicator ::is-selected="isSelected"
            :is-indicator-partially-hidden="$isIndicatorPartiallyHidden()" :idle-indicator="$getIdleIndicator()"
            :selected-indicator="$getSelectedIndicator()" class="fi-fo-radio-card__indicator" />
        @endif
        ...
        <input type="radio" {{ $inputAttributes->class(['hidden']) }} />
      </label>
    @endforeach
  </div>
</x-dynamic-component>